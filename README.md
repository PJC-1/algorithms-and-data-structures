...
>- When we put a multiple *nodes* together we refer to it as a **Linked List** because it literally is *a list of linked nodes*.
...
>- Every **node** has two parts to it. The first being a sort of *data bucket** assigned to it. The *data* that can be contained in it can be anything we want (i.e. string, number, array, object, etc...), it only has to be a **valid** javascript value can be contained in one of these *nodes*. The other part of the **node** is a reference to the *next* **node** along the **chain**.
